...
const getGenderSnapshot = (detail: ApiTarkamDetail, gender: GenderKey) => {
  const slot = gender === "male" ? toNumber(detail.male_slot) : toNumber(detail.female_slot);
  const players = gender === "male" ? toNumber(detail.male_players_count) : toNumber(detail.female_players_count);
  const completed = gender === "male" ? toNumber(detail.male_completed) : toNumber(detail.female_completed);
...
const getStreamUrl = (stream?: ApiStreaming | null) => stream?.embed?.trim() || stream?.url?.trim() || "";
...
const TarkamDetailsContent = ({ tarkamId }: { tarkamId?: number }) => {
  ...
  const heroImage = resolveImage(detail.image, detail.thumbnail, placeholderTeam) || placeholderTeam;
  ...
  const totalPlayers = formatNumber(detail.players_count ?? bundle.players.length);
  const totalGroups = formatNumber(detail.groups_count ?? bundle.groups.length);
  const totalContests = formatNumber(detail.contests_count ?? bundle.contests.length);
  const totalWinners = formatNumber(detail.winners_count ?? bundle.winners.length);
  const totalSessions = formatNumber(detail.sessions_count ?? bundle.sessions.length);
  const totalTimelines = formatNumber(detail.timelines_count ?? bundle.timelines.length);
  const totalStreams = formatNumber(detail.streamings_count ?? bundle.streamings.length);
  const highlightStats = [
    { label: "Teams", value: totalTeams, hint: `${formatNumber(filteredTeams.length)} tampil di filter aktif` },
    { label: "Players", value: totalPlayers, hint: `${formatNumber(filteredPlayers.length)} player sinkron` },
    { label: "Sessions", value: totalSessions, hint: `${updatedLabel} update terakhir` },
    { label: "Streams", value: totalStreams, hint: `${formatNumber(bundle.streamings.length)} kanal live` },
  ];
        ...
        <div className="container">
          <div className="tarkam-hero-grid">
            ...
            >
              <div className="tarkam-hero-media__content">
                <div className="tarkam-badge-row">
                  <span className="tarkam-badge">Week {detail.week || "-"}</span>
                ...
                <div className="tarkam-hero-media__footer">
                  <div>
                    ...
                    <strong>{formatCurrency(toNumber(detail.pool_price_m) + toNumber(detail.pool_price_f))}</strong>
                  </div>
                  <div>
            ...
            <article className="galactic-hover-card tarkam-panel">
              <Link className="tarkam-back-link" to={galacticRoutes.tarkamSchedule}>
                Kembali ke jadwal Tarkam
              </Link>
              ...
              <div className="tarkam-kpi-grid">
                {highlightStats.map((item) => (
                  <MiniStat key={item.label} label={item.label} value={item.value} hint={item.hint} />
                ))}
              </div>
              ...
              <div className="tarkam-inline-meta">
                <div className="tarkam-inline-meta__item">
                  <span>Transfer</span>
                  <strong>{detail.transfer_info || "Belum ada instruksi transfer."}</strong>
                </div>
                <div className="tarkam-inline-meta__item">
          ...
          <div className="galactic-hover-card tarkam-panel tarkam-panel--content">
            <div className="tarkam-tab-nav" role="tablist" aria-label="Navigasi detail Tarkam">
              {tabItems.map((tab) => (
                <button
                  key={tab.key}
                  type="button"
                    ...
                    >
                      <div className="tarkam-gender-card__head">
                        <div>
                          ...
                          <h3 className="tarkam-gender-card__title">{detail.title || "Tarkam"}</h3>
                        </div>
                        <span className="tarkam-badge">
                          {formatNumber(snapshot.slot - snapshot.remaining)} / {formatNumber(snapshot.slot)} slot
                        </span>
                      </div>

                      <div className="tarkam-meta-grid">
                        <MiniStat label="Date" value={formatDate(snapshot.date)} />
                        <MiniStat label="Time" value={snapshot.time || "TBA"} />
                        <MiniStat label="Sisa Slot" value={formatNumber(snapshot.remaining)} />
                        <MiniStat label="Players" value={formatNumber(snapshot.players)} />
                        <MiniStat label="Completed" value={formatNumber(snapshot.completed)} />
                        <MiniStat label="Pool" value={formatCurrency(snapshot.poolPrice)} />
                        <MiniStat label="MVP" value={snapshot.mvp || "-"} />
                        <MiniStat label="Status" value={statusLabel(detail.status)} />
                      </div>
                    </article>
                  ))}
                </div>
                  ...
                  <article className="galactic-hover-card tarkam-data-card">
                    ...
                    <div className="tarkam-detail-stack">
                      <div className="tarkam-detail-row">
                        ...
                        <strong>{formatNumber(bundle.streamings.length)}</strong>
                      </div>
                      <div className="tarkam-detail-row">
                    ...
                    <article className="galactic-hover-card tarkam-list-card">
                      ...
                      {filteredGroups.length ? (
                        filteredGroups.map((group) => (
                          <div className="tarkam-list-item" key={String(group.id)}>
                            <div className="tarkam-list-item__main">
                              <strong>{group.name || "Group"}</strong>
                              <span>{group.gender || "Divisi belum diatur"}</span>
                            </div>
                            <span className="tarkam-badge">{formatNumber(group.teams_count)} tim</span>
                          </div>
                        ))
                      ) : (
                        <InlineEmptyState
                      ...
                      {filteredTeams.length ? (
                        filteredTeams.map((team) => {
                          const members = getTeamMembers(team)
                            .map((member) => member.nickname || member.username || "Member")
                            .join(", ");

                          return (
                            <div className="tarkam-list-item" key={String(team.id)}>
                              <div className="tarkam-list-item__main">
                                <strong>
                                  {hasValue(team.id) ? (
                                ...
                                <span>{team.group?.name || team.gender || "Divisi belum ditetapkan"}</span>
                              </div>
                              <div className="tarkam-list-item__meta">
                                <span>{formatDate(team.date)}</span>
                                <span>{team.time || "TBA"}</span>
                              </div>
                              <p className="tarkam-inline-copy">{members || "Belum ada member yang terhubung."}</p>
                            </div>
                          );
                        })
                      ) : (
                      ...
                      {filteredPlayers.length ? (
                        <div className="tarkam-player-grid">
                          {filteredPlayers.map((player) => {
                            const playerKey = player.member?.slug || player.member?.id || player.id;
                            const playerName = player.member?.nickname || player.member?.username || "Player";

                            return (
                              <article className="tarkam-member-card tarkam-member-card--player" key={String(player.id)}>
                                <div className="tarkam-member-card__head">
                                  <img
                                    className="tarkam-member-card__avatar"
                                    src={resolveImage(player.member?.picture_url, placeholderPlayer) || placeholderPlayer}
                                    alt={playerName}
                                  />
                                  <div className="tarkam-member-card__identity">
                                    <h4>
                                      {hasValue(playerKey) ? (
                                        <Link to={buildPlayerDetailPath(playerKey || "")}>{playerName}</Link>
                                      ) : (
                                        playerName
                                      )}
                                    </h4>
                                    <p>{player.member?.club?.name || "Belum terhubung ke klub"}</p>
                                  </div>
                                  <span className={`tarkam-pill ${player.paid ? "is-active" : "is-idle"}`}>
                                ...
                                <div className="tarkam-player-meta">
                                  <div className="tarkam-detail-row">
                                    <span>Gender</span>
                                    <strong>{player.member?.gender || "-"}</strong>
                                  </div>
                                  <div className="tarkam-detail-row">
                                    <span>Kota</span>
                                    <strong>{player.member?.city || "-"}</strong>
                                  </div>
                                  <div className="tarkam-detail-row">
                                    <span>Tier</span>
                                    <strong>{player.member?.tier || "-"}</strong>
                                  </div>
                                  <div className="tarkam-detail-row">
                                    <span>Score</span>
                ...
                <div className="row">
                  {bundle.sessions.length ? (
                    bundle.sessions.map((relation) => {
                      ...
                      const sessionLabel = session?.sesi ?? relation.session_fk ?? "-";
                        ...
                        <div className="col-lg-4 col-md-6 sm-padding" key={String(relation.id || relation.session_fk)}>
                          <article className="galactic-hover-card tarkam-session-card">
                            <div className="tarkam-session-card__head">
                              <div>
                                <div className="tarkam-eyebrow">Session {sessionLabel}</div>
                                <h3>Distribusi poin sesi</h3>
                              </div>
                              <span className={`tarkam-pill ${statusClass(session?.status)}`}>
                                {statusLabel(session?.status)}
                              </span>
                            </div>

                            <div className="tarkam-session-points">{formatNumber(session?.point)} poin</div>
                            ...
                            <div className="tarkam-detail-stack">
                              ...
                              <div className="tarkam-detail-row">
                                <span>Created</span>
                                <strong>{formatDateTime(session?.created_at || relation.created_at)}</strong>
                              </div>
                              <div className="tarkam-detail-row">
                                <span>Updated</span>
                                <strong>{formatDateTime(session?.updated_at || relation.updated_at)}</strong>
                              </div>
                            </div>
                ...
                <div className="row">
                  {bundle.timelines.length ? (
                    bundle.timelines.map((relation) => {
                      ...
                      const actor = timeline?.member?.nickname || timeline?.member?.username || timeline?.club?.name || "Tarkam";
                      const actorSubtext =
                        timeline?.club?.name ||
                        (timeline?.member?.club?.name ? `Klub ${timeline.member.club.name}` : "Aktivitas umum Tarkam");
                        ...
                        <div className="col-lg-6 sm-padding" key={String(relation.id || relation.timeline_fk || relation.session_fk)}>
                          <article className="galactic-hover-card tarkam-timeline-card">
                            <div className="tarkam-timeline-card__head">
                              <div>
                                <div className="tarkam-eyebrow">Timeline Event</div>
                                <h3>{actor}</h3>
                                <p className="tarkam-inline-copy">{actorSubtext}</p>
                              </div>
                              <span className="tarkam-badge">
                                {timeline?.session?.sesi !== undefined ? `Session ${timeline.session.sesi}` : "General"}
                              </span>
                            </div>

                            <p className="tarkam-timeline-card__description">
                              {timeline?.description || "Belum ada deskripsi timeline yang disimpan."}
                            </p>

                            <div className="tarkam-detail-stack">
                              <div className="tarkam-detail-row">
                                <span>Created</span>
                                <strong>{formatDateTime(timeline?.created_at || relation.created_at)}</strong>
                              </div>
                              <div className="tarkam-detail-row">
                                <span>Updated</span>
                                <strong>{formatDateTime(timeline?.updated_at || relation.updated_at)}</strong>
                              </div>
                            </div>
                          </article>
                        </div>
                ...
                {bundle.streamings.length ? (
                  <div className="tarkam-stream-grid">
                    {bundle.streamings.map((stream) => {
                      ...
                      const streamImage = resolveImage(stream.thumbnail, placeholderVideoThumb) || placeholderVideoThumb;

                      return (
                        <article className="galactic-hover-card tarkam-stream-card" key={String(stream.id)}>
                          <div className="tarkam-stream-card__thumb">
                            <img src={streamImage} alt={stream.title || "Streaming Tarkam"} />
                          </div>
                          <div className="tarkam-stream-card__body">
                            <div className="tarkam-eyebrow">Live Coverage</div>
                            <h3>{stream.title || "Streaming"}</h3>
                            <p>{stream.description || "Streaming untuk Tarkam ini belum memiliki deskripsi tambahan."}</p>
                            {streamUrl ? (
                              <VideoStreemButton href={streamUrl} normalizeFacebook />
                            ) : (
                              <span className="tarkam-inline-copy">Tautan streaming belum tersedia.</span>
                            )}
                          </div>
                        </article>
                      );
                  ...
                  <article className="galactic-hover-card tarkam-list-card">
                    ...
                    {filteredContests.length ? (
                      filteredContests.map((contest) => (
                        <div className="tarkam-list-item" key={String(contest.id)}>
                          <div className="tarkam-list-item__main">
                          ...
                          <p className="tarkam-inline-copy">
                            Winner: {contest.winnerTeam?.name || "Belum ditentukan"}
                          </p>
                        </div>
                      ))
                    ) : (
                  ...
                  <article className="galactic-hover-card tarkam-list-card">
                    ...
                    {filteredWinners.length ? (
                      filteredWinners.map((winner) => (
                        <div className="tarkam-list-item" key={String(winner.id)}>
                          <div className="tarkam-list-item__main">
                            <strong>
                          ...
                          <div className="tarkam-list-item__meta">
                            <span>{winner.team?.name || "Tim belum ditautkan"}</span>
                            <span>{formatDateTime(winner.created_at)}</span>
                          </div>
                          <p className="tarkam-inline-copy">{winner.prize || "Hadiah belum dicatat."}</p>
                        </div>
                      ))
                    ) : (
                      <InlineEmptyState
