import SEO from "@/components/SEO";
import { PageHeader } from "@/galactic/common";

function TermsOfService() {
  return (
    <>
      <SEO
        title="Syarat Penggunaan – Bot Discord Tarkam"
        description="Aturan dan ketentuan pakai Bot Discord Tarkam. Baca dulu sebelum pakai."
        keywords={[
          "syarat penggunaan",
          "bot discord",
          "tarkam",
          "ketentuan",
          "legal",
        ]}
        type="website"
      />
      <PageHeader
        eyebrow="Syarat & Ketentuan"
        title="Syarat Penggunaan – Bot Discord Tarkam"
        description="Aturan dan ketentuan pakai Bot Discord Tarkam. Baca dulu sebelum pakai."
      />
      <section className="about-section padding">
        <div className="container">
          <div className="row align-items-center">
            <div className="col-md-12 sm-padding">
              <div className="about-content">
                <div className="section-heading">
                  <h3>Syarat & Ketentuan - Last Updated: March 16, 2026</h3>
                  <h2>
                    Syarat dan Ketentuan penggunaan bot <span>Discord</span> dan{" "}
                    <span>WhatsApp</span>
                  </h2>
                  <p>
                    Halaman ini ngejelasin aturan main dan ketentuan pakai Bot
                    Discord Tarkam ("Bot", "Layanan") yang dijalankan oleh Dist
                    ("Developer", "kami").
                  </p>
                  <p className="mt-20">
                    Dengan mengundang, mengakses, atau memakai Bot lewat
                    Discord/WhatsApp, kamu setuju sama syarat ini. Kalau nggak
                    setuju, berhenti pakai Bot sekarang juga.
                  </p>
                  <h3 className="mt-20">1. Deskripsi Layanan</h3>
                  <p>
                    Tarkam adalah bot Discord yang dirancang untuk bantu server
                    dengan fitur-fitur seperti:
                  </p>
                  <ul>
                    <li>Alat moderasi server</li>
                    <li>Perintah otomatis dan utility</li>
                    <li>Fitur engagement komunitas</li>
                    <li>Fungsi hiburan atau game-related</li>
                    <li>Integrasi dengan layanan atau API eksternal</li>
                  </ul>
                  <p className="mt-20">
                    Fitur bisa diubah, ditambah, atau dihapus kapan saja tanpa
                    pemberitahuan terlebih dahulu.
                  </p>
                  <p className="mt-20">
                    Untuk info lebih lanjut tentang Bot, kunjungi:{" "}
                    <a
                      href="https://tarkam.fun"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      https://tarkam.fun
                    </a>
                  </p>

                  <h3 className="mt-20">2. Kelayakan</h3>
                  <p>Untuk pakai Bot, kamu harus:</p>
                  <ul>
                    <li>
                      Berusia minimal 13 tahun atau sesuai ketentuan umur
                      Discord.
                    </li>
                    <li>Mematuhi semua hukum dan aturan yang berlaku.</li>
                    <li>
                      Ikut Discord Terms of Service dan Discord Community
                      Guidelines.
                    </li>
                  </ul>
                  <p className="mt-20">
                    Kalau nggak taat, akses kamu bisa dibatasi atau Bot bisa
                    dicabut dari server.
                  </p>

                  <h3 className="mt-20">3. Penggunaan Wajar</h3>
                  <p>
                    Pengguna setuju untuk tidak menyalahgunakan Bot, termasuk
                    tapi tidak terbatas pada:
                  </p>
                  <ul>
                    <li>Melanggar aturan Discord atau ketentuan ini</li>
                    <li>Mengganggu, mengancam, atau menyakiti pengguna lain</li>
                    <li>Mengirim spam atau perintah jahat</li>
                    <li>Eksploit bug atau mencoba akses tidak sah</li>
                    <li>Menggunakan Bot untuk aktivitas ilegal</li>
                    <li>Mengganggu fungsi normal Bot</li>
                  </ul>
                  <p className="mt-20">
                    Developer berhak suspend, blokir, atau banned user atau
                    server yang melanggar aturan.
                  </p>

                  <h3 className="mt-20">4. Pengumpulan Data</h3>
                  <p>
                    Agar layanan berjalan, Bot bisa mengumpulkan dan memproses
                    data teknis terbatas seperti:
                  </p>
                  <ul>
                    <li>ID pengguna Discord</li>
                    <li>ID server (Guild)</li>
                    <li>ID channel</li>
                    <li>Log penggunaan perintah</li>
                    <li>Setelan konfigurasi server</li>
                  </ul>
                  <p className="mt-20">
                    Data ini hanya dipakai untuk operasional, debugging, dan
                    ningkatin layanan.
                  </p>
                  <p className="mt-20">
                    Kami tidak menjual atau membagikan data pribadi ke pihak
                    ketiga kecuali jika diwajibkan oleh hukum.
                  </p>

                  <h3 className="mt-20">5. Penyimpanan Data & Retensi</h3>
                  <p>
                    Beberapa data bisa disimpan sementara atau dipertahankan
                    sesuai fitur yang aktif di server.
                  </p>
                  <p className="mt-20">
                    Administrator server atau pengguna bisa minta data dihapus
                    lewat support server:
                  </p>
                  <p className="mt-20">
                    Support Server:{" "}
                    <a
                      href="https://discord.gg/fnRhRkxTyC"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      https://discord.gg/fnRhRkxTyC
                    </a>
                  </p>

                  <h3 className="mt-20">6. Ketersediaan Layanan</h3>
                  <p>
                    Bot disediakan apa adanya dan tidak selalu terjamin
                    tersedia.
                  </p>
                  <p className="mt-20">Kami tidak menjamin bahwa layanan akan:</p>
                  <ul>
                    <li>Selalu tersedia tanpa gangguan</li>
                    <li>Bebas dari bug atau error</li>
                    <li>Memenuhi harapan semua pengguna</li>
                  </ul>
                  <p className="mt-20">
                    Developer berhak mengubah, menangguhkan, atau menghentikan
                    layanan kapan saja tanpa pemberitahuan.
                  </p>

                  <h3 className="mt-20">7. Batas Tanggung Jawab</h3>
                  <p>
                    Sejauh diizinkan hukum, Dist tidak bertanggung jawab atas:
                  </p>
                  <ul>
                    <li>Hilangnya atau rusaknya data</li>
                    <li>Salah konfigurasi server</li>
                    <li>Gangguan atau jeda layanan</li>
                    <li>
                      Kerugian tidak langsung yang muncul akibat penggunaan Bot
                    </li>
                  </ul>
                  <p className="mt-20">Penggunaan Bot adalah risiko kamu sendiri.</p>

                  <h3 className="mt-20">8. Penghentian</h3>
                  <p>Kami berhak:</p>
                  <ul>
                    <li>Membatasi akses untuk pengguna atau server tertentu</li>
                    <li>Mengeluarkan Bot dari server mana pun</li>
                    <li>Menangguhkan atau menghentikan layanan sepenuhnya</li>
                  </ul>
                  <p className="mt-20">
                    Pengguna bisa berhenti pakai Bot kapan saja dengan
                    menghapusnya dari server Discord.
                  </p>

                  <h3 className="mt-20">9. Perubahan Syarat</h3>
                  <p>Kami bisa memperbarui syarat ini secara berkala.</p>
                  <p>
                    Penggunaan Bot setelah perubahan dipublikasi dianggap
                    sebagai penerimaan syarat yang direvisi.
                  </p>
                  <p className="mt-20">Update bisa diumumkan di:</p>
                  <ul>
                    <li>
                      <a
                        href="https://tarkam.fun"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://tarkam.fun
                      </a>
                    </li>
                    <li>Support server resmi</li>
                  </ul>

                  <h3 className="mt-20">10. Kontak & Dukungan</h3>
                  <p>
                    Kalau ada pertanyaan tentang syarat ini, hubungi kami lewat:
                  </p>
                  <p className="mt-20">
                    Website:{" "}
                    <a
                      href="https://tarkam.fun"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      https://tarkam.fun
                    </a>
                  </p>
                  <p className="mt-20">
                    Support Server:{" "}
                    <a
                      href="https://discord.gg/fnRhRkxTyC"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      https://discord.gg/fnRhRkxTyC
                    </a>
                  </p>
                  <p className="mt-20">Developer: Dist</p>

                  <h3 className="mt-20">11. Hukum yang Berlaku</h3>
                  <p>
                    Syarat ini diatur oleh hukum Indonesia dan yurisdiksi
                    Jakarta.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}

export default TermsOfService;
